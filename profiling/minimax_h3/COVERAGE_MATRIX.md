...
| QKV-A2A overlap | compute/comm overlap,资源节流 | 是 | 是 (vB region -5.4%,step ~0.5-0.6%) | — | 部分（compile 消 fp32 链） | 是 | NCCL_MAX_CTAS=8 A/B 待跑 | **INVESTIGATE** |
| producer→consumer direct layout (fp32 attn 区归一 bf16) | dtype 归一 | 是 | — | — | 部分（compile 消 cast soup) | 是 | 冻结（人工分析占线） | **INVESTIGATE** |
| all-stream bubble / allocator churn | expandable_segments / FSDP 主机暴露 | 是 (14.56%/step,NSys 交叉) | 未实测 | — | 部分（compile 消 183ms FSDP/183ms) | 是 | `expandable_segments:False` A/B 待跑 | **INVESTIGATE** |
...
| torch.compile+AdaLN overlap 复杂 pipeline | — | — | — | — | — | — | 超本轮范围 | **DROP** |
...
所有 baseline cost ≥0.5% 的项都已：(a) 被 compile subsume,(b) 被 mature backend 证明不可替换，(c) 被 E2E 证实负收益，(d) 被本轮显式禁，或 (e) 冻结等待 dtype 实验。
